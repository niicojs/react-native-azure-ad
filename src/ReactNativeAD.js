...
const defaultTokenUrl = 'https://login.microsoftonline.com/organizations/oauth2/v2.0/token';
...
export default class ReactNativeAD {
  ...
  static ResourceOwnerPasswordCredential(
    data: ResourceOwnerPasswordCredential
  ) {
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let resource in data) {
        if (resource && data[resource]) {
          ...
          console.warn(
            `counld not save credential for ${resource}=${
              data[resource]
            } for its key/value is null/undefeind.`
          );
      }
  ...
  checkCredential(resourceId: string): Promise<ReactNativeADCredential | null> {
    ...
    return new Promise((resolve, reject) => {
      ...
      log.verbose(
        `checkCredential:'${resourceId} cached at ${resourceKey}=${cachedCred}`
      );
      // When in memory context not found, check AsyncStorage.
      if (!cachedCred || cachedCred === void 0) {
        try {
          AsyncStorage.getItem(resourceKey).then(credStr => {
